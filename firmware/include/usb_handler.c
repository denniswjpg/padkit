      ...
      switch(SetupReq) {                          // request ccfType
        case USB_GET_DESCRIPTOR:
          switch(USB_setupBuf->wValueH) {
            ...
            // PadKit local patch: two HID interfaces, so the report descriptor is
            // selected by interface number (wIndex): 0 = keyboard, 1 = vendor.
            case USB_DESCR_TYP_REPORT:
              if(USB_setupBuf->wValueL != 0) { len = 0xff; break; }
              if(USB_setupBuf->wIndexL == 1) {
                pDescr = VendorReportDescr;
                len = VendorReportDescrLen;
              } else {
                pDescr = KbdReportDescr;
                len = KbdReportDescrLen;
              }
              break;

            // PadKit local patch: answer per-interface HID (class) descriptor
            // requests (0x21). Windows in particular asks for these directly.
            case USB_DESCR_TYP_HID:
              if(USB_setupBuf->wIndexL == 1)
                pDescr = (uint8_t*)&CfgDescr.hid1;
              else
                pDescr = (uint8_t*)&CfgDescr.hid0;
              len = sizeof(USB_HID_DESCR);
              break;
            #endif
