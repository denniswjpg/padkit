...
//  * Raw HID keycode + modifier emit for the v0.2 remappable keymap. Bypasses the
//  * ASCII map and the 128/136 escape scheme so SET_KEY can carry any standard
//  * HID Keyboard/Keypad usage (page 0x07, up to 0xFF) plus an arbitrary modifier
//  * bitmask, which the escape scheme could not represent (136+code overflows for
//  * code >= 0x78). `mod` is the standard modifier bitmask (bit0 LCtrl ...);
//  * `code` is a page-0x07 usage, 0 = no keycode (modifier-only or disabled).
//  */
// ===================================================================================
void KBD_pressRaw(uint8_t mod, uint8_t code) {
  uint8_t i;
  KBD_report[1] |= mod;                           // OR in modifier bits
  if(code) {
    for(i=3; i<9; i++) if(KBD_report[i] == code) { KBD_sendReport(); return; }
    for(i=3; i<9; i++) if(KBD_report[i] == 0) { KBD_report[i] = code; break; }
  }
  KBD_sendReport();
}

void KBD_releaseRaw(uint8_t mod, uint8_t code) {
  uint8_t i;
  KBD_report[1] &= (uint8_t)~mod;                 // clear this control's modifier bits
  if(code) for(i=3; i<9; i++) if(KBD_report[i] == code) KBD_report[i] = 0;
  KBD_sendReport();
}
