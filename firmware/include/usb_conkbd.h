...
// PadKit local patch: raw HID keycode+modifier emit for the remappable keymap.
void KBD_pressRaw(uint8_t mod, uint8_t code);   // press modifier bits + keycode
void KBD_releaseRaw(uint8_t mod, uint8_t code); // release modifier bits + keycode
