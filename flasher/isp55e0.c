...
static void open_usb_device(struct device *dev)
{
	...
	/* This bootloader can come up with its bulk endpoints halted (seen on a
	 * CH552 macropad after repeated probing), which makes every transfer
	 * return EIO. Clearing the halt on both endpoints costs nothing when they
	 * are already fine and rescues the session when they are not. */
	libusb_clear_halt(dev->usb_h, EP_OUT);
	libusb_clear_halt(dev->usb_h, EP_IN);
}
#endif
...
static void reboot_device(struct device *dev)
{
	...
	if (ret) {
		/* This CH552's 2.50 ROM flashes+verifies fine but never ACKs the
		 * reboot command. The firmware is already written and verified at
		 * this point, so a missing reboot ACK must not fail the run — a
		 * power cycle boots the new firmware regardless. */
		warnx("Reboot not acknowledged (harmless) — unplug/replug to boot the new firmware");
		return;
	}
